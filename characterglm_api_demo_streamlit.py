...
API_KEY: str = "e69e9e61b90b271bbdc811ea3b74f43c.9RxK963ZRmlQOzmB"
...
with st.container():
    ...
    with col3:
        st.text_input(label="画风风格", value="二次元风格", key="fengge_info", on_change=lambda : st.session_state["meta"].update(fengge_info=st.session_state["fengge_info"]), help="生成图片时用，默认是二次元风格，不可为空。")


def verify_meta() -> bool:
    # 检查`角色名`和`角色人设`是否空，若为空，则弹出提醒
    if st.session_state["meta"]["bot_name"] == "" or st.session_state["meta"]["bot_info"] == "" or st.session_state["meta"]["fengge_info"] == "" :
        st.error("角色名、角色人设、画风风格不能为空")
        return False
    else:
        return True
...
def draw_new_image():
    ...
    # TODO: 加上风格选项
    image_prompt = st.session_state["meta"]["fengge_info"] +'。'+ image_prompt.strip()
